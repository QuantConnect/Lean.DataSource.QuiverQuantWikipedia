...
namespace QuantConnect.DataProcessing
{
    ...
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            QuiverWikipediaDataDownloader instance = null;
            try
            {
                // Pass in the values we got from the configuration into the downloader/converter.
                instance = new QuiverWikipediaDataDownloader(destinationDataFolder);
            }
            catch (Exception err)
            {
                Log.Error(err, $"QuantConnect.DataProcessing.Program.Main(): The downloader/converter for {QuiverWikipediaDataDownloader.VendorDataName} {QuiverWikipediaDataDownloader.VendorDataName} data failed to be constructed");
                Environment.Exit(1);
            }
            ...
            try
            {
                ...
                if (!success)
                {
                    Log.Error($"QuantConnect.DataProcessing.Program.Main(): Failed to download/process {QuiverWikipediaDataDownloader.VendorName} {QuiverWikipediaDataDownloader.VendorDataName} data");
                    Environment.Exit(1);
                }
            }
            catch (Exception err)
            {
                Log.Error(err, $"QuantConnect.DataProcessing.Program.Main(): The downloader/converter for {QuiverWikipediaDataDownloader.VendorDataName} {QuiverWikipediaDataDownloader.VendorDataName} data exited unexpectedly");
                Environment.Exit(1);
            }
            finally
            {
