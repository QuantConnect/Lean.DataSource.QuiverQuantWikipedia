...
{
    /// <summary>
    /// QuiverWikipediaDataDownloader implementation. https://www.quiverquant.com/
    /// </summary>
    public class QuiverWikipediaDataDownloader : IDisposable
    {
                        ...
                        HttpRequester($"historical/{VendorDataName}/{ticker}")
                            .ContinueWith(
                                y =>
                                {
                                    ...
                                    {
                                        Log.Error(
                                            $"QuiverWikipediaDataDownloader.Run(): Failed to get data for {company}");
                                        return;
                                    }
                                    ...
                                    {
                                        Log.Trace(
                                            $"QuiverWikipediaDataDownloader.Run(): {percentageDone.ToStringInvariant("P2")} complete");
            ...
            for (var retries = 1; retries <= _maxRetries; retries++)
            {
                try
                {
                    using (var client = new HttpClient())
                    {
                        ...
                        {
                            Log.Error($"QuiverWikipediaDataDownloader.HttpRequester(): Files not found at url: {Uri.EscapeUriString(url)}");
                ...
                {
                    Log.Error(e, $"QuiverWikipediaDataDownloader.HttpRequester(): Error at HttpRequester. (retry {retries}/{_maxRetries})");
                    Thread.Sleep(1000);
                }
            }
